...
<div class="space-y-4">
    <div class="flex justify-between items-center flex-wrap gap-3">
        <h2 class="text-xl font-bold flex items-center gap-2"><i class="fi fi-rr-restaurant text-blue-600 flex items-center"></i> Data Pramusaji</h2>
        <button onclick="showModalTambahPramusaji()" class="px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition shadow-md flex items-center gap-1.5 text-sm">
            <i class="fi fi-rr-plus flex items-center"></i> Tambah Pramusaji
        </button>
    </div>

    <div class="bg-white p-4 rounded-xl shadow flex flex-wrap gap-3 items-end mb-4">
        <div class="flex-1 min-w-[200px]">
            <label class="block text-xs font-bold text-gray-400 mb-1 uppercase">Cari Pramusaji (Nama/NIP/Username)</label>
            <input type="text" id="searchPramusajiInput" class="w-full border border-gray-200 rounded-lg p-2 text-gray-700 focus:border-blue-500 focus:ring-blue-500 focus:ring-1" placeholder="Cari pramusaji...">
        </div>
        <div class="w-64">
            <label class="block text-xs font-bold text-gray-400 mb-1 uppercase">Filter Bangsal</label>
            <select id="filterBangsalPramusajiSelect" class="w-full border border-gray-200 rounded-lg p-2 text-gray-700 focus:border-blue-500 focus:ring-blue-500 focus:ring-1">
        ...
        <div>
            <button id="resetPramusajiFilterBtn" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg border border-gray-200 text-gray-700 font-medium transition">
                Reset
            </button>
        </div>
    </div>
    ...
    <div class="overflow-x-auto bg-white rounded-2xl shadow-md border border-slate-100 p-4">
        <table id="tabelPramusaji" class="w-full text-sm">
        ...
        <div class="flex justify-between items-center p-4 border-t border-slate-100 bg-slate-50/50 gap-4 flex-wrap">
            <div class="flex items-center gap-2">
                <label for="perPageSelectPramusaji" class="text-sm text-slate-600 font-medium">Baris per halaman:</label>
                <select id="perPageSelectPramusaji" class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
            ...
            <div id="paginationInfo" class="text-sm text-slate-600 font-medium">
                Menampilkan 1 sampai 10 dari <span id="totalRows">0</span> pramusaji
            </div>
...
<!-- Modal Form -->
<div id="pramusajiModal" class="fixed inset-0 bg-black/50 hidden flex justify-center items-center z-50 backdrop-blur-sm transition-all duration-300">
    <div class="bg-white rounded-2xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl relative mx-4 transform scale-95 transition-transform duration-300">
        ...
        <h3 id="modalTitle" class="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2"><i class="fi fi-rr-restaurant text-blue-600 flex items-center"></i> Tambah Pramusaji</h3>
        
        <form id="pramusajiForm" enctype="multipart/form-data" onsubmit="simpanPramusaji(event)">
            <?= csrf_field() ?>
            <input type="hidden" id="id_pramusaji" name="id_pramusaji">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <input type="text" id="nama_pramusaji" name="nama_pramusaji" required class="w-full border border-gray-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                    </div>
                    <div>
...
class PramusajiTable {
    ...
    renderRows() {
        ...
        if (paginatedRows.length === 0) {
            this.tbody.innerHTML = '<tr><td colspan="6" class="p-4 text-center text-slate-500">Tidak ada data pramusaji ditemukan.</td></tr>';
            return;
        }
        
        this.tbody.innerHTML = paginatedRows.map(row => `
            <tr>
                <td class="p-4 font-bold text-slate-800 align-middle">${row.nama_pramusaji || '-'}</td>
                ...
                <td class="p-4 text-center align-middle whitespace-nowrap">
                    <button onclick="editPramusajiModal('${row.id_pramusaji}')" class="text-xs bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg font-bold hover:bg-blue-100 transition-colors mr-2 inline-flex items-center gap-1">
                        <i class="fi fi-rr-edit"></i> Edit
                    </button>
                    <button onclick="hapusPramusaji('${row.id_pramusaji}')" class="text-xs bg-red-50 text-red-600 px-3 py-1.5 rounded-lg font-bold hover:bg-red-100 transition-colors inline-flex items-center gap-1">
    ...
    updatePaginationInfo() {
        if (this.paginationInfo && this.totalRowsSpan) {
            ...
            if (total === 0) {
                this.paginationInfo.textContent = 'Menampilkan 0 pramusaji';
            } else {
                this.paginationInfo.innerHTML = `Menampilkan ${start} sampai ${end} dari <span id="totalRows">${total}</span> pramusaji`;
            }
        }
    }
    ...
    renderPaginationButtons() {
        ...
        if (this.currentPage > 1) {
            html += `<button onclick="tablePramusaji.goToPage(${this.currentPage - 1})" class="px-3 py-1.5 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg text-sm font-medium transition">← Sebelumnya</button>`;
        }
        ...
        if (startPage > 1) {
            html += `<button onclick="tablePramusaji.goToPage(1)" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-sm font-medium transition">1</button>`;
            if (startPage > 2) {
        ...
        for (let i = startPage; i <= endPage; i++) {
            if (i === this.currentPage) {
                ...
                html += `<button onclick="tablePramusaji.goToPage(${i})" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-sm font-medium transition">${i}</button>`;
            }
        }
        ...
        if (endPage < totalPages) {
            ...
            html += `<button onclick="tablePramusaji.goToPage(${totalPages})" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-sm font-medium transition">${totalPages}</button>`;
        }
        
        if (this.currentPage < totalPages) {
            html += `<button onclick="tablePramusaji.goToPage(${this.currentPage + 1})" class="px-3 py-1.5 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg text-sm font-medium transition">Selanjutnya →</button>`;
        }
...
document.addEventListener('DOMContentLoaded', function() {
    tablePramusaji = new PramusajiTable('#tabelPramusaji', {
        searchInput: document.getElementById('searchPramusajiInput'),
        filterSelect: document.getElementById('filterBangsalPramusajiSelect'),
        resetBtn: document.getElementById('resetPramusajiFilterBtn'),
        perPageSelect: document.getElementById('perPageSelectPramusaji'),
        paginationButtons: document.getElementById('paginationButtons'),
        paginationInfo: document.getElementById('paginationInfo'),
        totalRowsSpan: document.getElementById('totalRows'),
        apiUrl: '<?= site_url('pramusaji/getData') ?>'
    });
});
...
function showModalTambahPramusaji() {
    ...
    const modal = document.getElementById('pramusajiModal');
...
function editPramusajiModal(id) {
    fetch(`<?= base_url('pramusaji/edit/') ?>${id}`)
        .then(response => response.json())
        .then(data => {
            document.getElementById('modalTitle').innerHTML = '<i class="fi fi-rr-edit text-blue-600 flex items-center"></i> Edit Pramusaji';
            document.getElementById('id_pramusaji').value = data.id_pramusaji;
            document.getElementById('nama_pramusaji').value = data.nama_pramusaji;
...
function simpanPramusaji(event) {
    ...
    const formData = new FormData(document.getElementById('pramusajiForm'));
    
    fetch(`<?= base_url('pramusaji/save') ?>`, {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
...
function hapusPramusaji(id) {
    Swal.fire({
        title: 'Apakah Anda yakin?',
        text: "Data pramusaji ini akan dihapus secara permanen!",
        icon: 'warning',
...
function validateForm() {
    const idPramusaji = document.getElementById('id_pramusaji').value;
    const isEditMode = idPramusaji !== '';
    
    const namaPramusaji = document.getElementById('nama_pramusaji').value.trim();
    const username = document.getElementById('username').value.trim();
    const idBangsal = document.getElementById('id_bangsal').value;
    const password = document.getElementById('password').value;
    const fileInput = document.getElementById('photo');
    
    if (!namaPramusaji || !username || !idBangsal) {
        Swal.fire('Error', 'Nama, Username, dan Bangsal wajib diisi!', 'error');
        return false;
    }

    if(!isEditMode && !password.trim()) {
        Swal.fire('Error', 'Password wajib diisi untuk pramusaji baru!', 'error');
        return false;
    }
