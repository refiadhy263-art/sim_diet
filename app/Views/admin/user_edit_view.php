...
<?= $this->section('title') ?>
<title>Edit Pengguna | SIMDIET</title>
<?= $this->endSection() ?>
...
<div class="space-y-6 max-w-2xl mx-auto">
    ...
    <div class="flex justify-between items-center">
        <h2 class="text-xl font-bold flex items-center gap-2">
            <i class="fi fi-rr-user-pen text-blue-600 flex items-center"></i> Edit Pengguna
        </h2>
    ...
    <div class="bg-white rounded-2xl shadow-md border border-slate-100 p-6">
        <form method="post" action="<?= base_url('user/update'); ?>" class="space-y-5">
            <?= csrf_field(); ?>
            <input type="hidden" name="id_user" value="<?= $user['id_user'] ?>">

            <div>
                <label class="block text-sm font-semibold text-gray-600 mb-1">Username</label>
                <input type="text" name="username" required class="w-full border border-gray-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Masukkan username" value="<?= esc($user['username']) ?>">
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-600 mb-1">Nama Lengkap</label>
                <input type="text" name="nama_user" required class="w-full border border-gray-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" placeholder="Masukkan nama lengkap" value="<?= esc($user['nama_user']) ?>">
            </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <?php foreach ($users as $tampil) : ?>
                        <label class="flex items-center gap-2 cursor-pointer bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 transition">
                            <input type="radio" name="id_kategori_user" value="<?= $tampil['id_kategori_user'] ?>" class="text-blue-600 focus:ring-blue-500" <?= $user['id_kategori_user'] == $tampil['id_kategori_user'] ? 'checked' : '' ?>>
