 ...
 public function __construct()
{
    ...
    public function detail_bangsal($id_bangsal)
    {
        ...
        $jam_menit = date('H:i'); // Contoh format: '14:30'
        $tanggal   = date('Y-m-d');
        
        $jadwalAktif = 'Belum ada jadwal';
        $is_waktu_proses = false; // Mengontrol apakah tombol boleh diklik
        $shift_sekarang  = ''; // Pengenal shift untuk keperluan reset

        // 2. Logika Jendela Waktu (Time Window)
        if ($jam_menit >= '05:30' && $jam_menit < '10:00') {
            $shift_sekarang  = 'pagi';
            $is_waktu_proses = true;
            $jadwalAktif     = 'Makan Pagi (05:30 - 09:59)'; 
            $next            = 'Makan Siang (10:00)';
            
        } elseif ($jam_menit >= '10:00' && $jam_menit < '15:30') {
            $shift_sekarang  = 'siang';
            $is_waktu_proses = true;
            $jadwalAktif     = 'Makan Siang (10:00 - 15:29)'; 
            $next            = 'Makan Malam (15:30)';
            
        } elseif ($jam_menit >= '15:30' || $jam_menit < '05:30') {
            // Makan malam aktif dari 15:30 sampai pagi (05:29)
            $shift_sekarang  = 'malam';
            $is_waktu_proses = true;
            $jadwalAktif     = 'Makan Malam (15:30 - 05:29)'; 
            $next            = 'Makan Pagi (05:30)';
        }

        $kode_shift_sekarang = $tanggal . '_' . $shift_sekarang; // Contoh: 2026-06-05_siang

        // 1. Ambil data bangsal ini dari database
  
        $bangsal  = $this->bangsal->find($id_bangsal);

        // 2. LOGIKA AUTO-RESET MENGGUNAKAN DATABASE
        // Jika shift di database beda dengan shift saat ini, lakukan reset!
        if ($bangsal['reset_order'] !== $kode_shift_sekarang && $is_waktu_proses) {
            
            // A. Reset status semua pasien di bangsal ini menjadi '0'
            
            $this->pasien->getResetOrderBangsal($id_bangsal);
            // B. Update kolom shift_terakhir di tabel bangsal agar tidak di-reset lagi oleh perawat lain
            $this->bangsal->update($id_bangsal, ['reset_order' => $kode_shift_sekarang]);

             // C. Log aktivitas reset
            $this->logs->saveLog('Reset Order', "Melakukan reset order otomatis untuk bangsal " . $bangsal['nama_bangsal'] . " pada jadwal " . $jadwalAktif);
        }
